...
export function createRoutes(
  ...
  const extractNativeSignalsFromParsedAlert = (parsedAlert: any): string[] => {
    ...
    if (alarmFlags.gnssModuleFailure) signals.push('jt808_gnss_module_failure');
    if (alarmFlags.gnssAntennaDisconnected) signals.push('jt808_gnss_antenna_disconnected');
    if (alarmFlags.gnssAntennaShortCircuit) signals.push('jt808_gnss_antenna_short_circuit');
    if (alarmFlags.terminalPowerUndervoltage) signals.push('jt808_terminal_power_undervoltage');
    if (alarmFlags.terminalPowerFailure) signals.push('jt808_terminal_power_failure');
    if (alarmFlags.terminalDisplayFailure) signals.push('jt808_terminal_display_failure');
    if (alarmFlags.ttsModuleFailure) signals.push('jt808_tts_module_failure');
    if (alarmFlags.cameraFailure) signals.push('jt808_camera_failure');
    if (alarmFlags.transportIcCardModuleFailure) signals.push('jt808_transport_ic_card_module_failure');
    ...
    if (alarmFlags.vibrationAlarm) signals.push('jt808_vibration_alarm');
    if (alarmFlags.lightAlarm) signals.push('jt808_light_alarm');
    if (alarmFlags.magneticInductiveAlarm) signals.push('jt808_magnetic_inductive_alarm');
    if (alarmFlags.accumulatedDrivingTimeAlarm) signals.push('jt808_accumulated_driving_time_alarm');
    if (alarmFlags.overtimeParking) signals.push('jt808_overtime_parking');
    if (alarmFlags.areaEntryExitAlarm) signals.push('jt808_area_entry_exit_alarm');
    if (alarmFlags.routeEntryExitAlarm) signals.push('jt808_route_entry_exit_alarm');
    if (alarmFlags.routeTravelTimeAlarm) signals.push('jt808_route_travel_time_alarm');
    if (alarmFlags.routeDeviationAlarm) signals.push('jt808_route_deviation_alarm');
    if (alarmFlags.vssFailure) signals.push('jt808_vss_failure');
    if (alarmFlags.abnormalFuelCapacity) signals.push('jt808_abnormal_fuel_capacity');
    if (alarmFlags.vehicleTheft) signals.push('jt808_vehicle_theft');
    if (alarmFlags.illegalIgnition) signals.push('jt808_illegal_ignition');
    if (alarmFlags.illegalDisplacement) signals.push('jt808_illegal_displacement');
    ...
    if (alarmFlags.illegalDoorOpenAlarm) signals.push('jt808_illegal_door_open_alarm');
